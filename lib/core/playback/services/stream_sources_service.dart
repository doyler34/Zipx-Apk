...
class StreamSourcesService {
  ...
  /// On the player path (`strict: false`), the AIOStreams request retries a
  /// couple of times on a thrown error (timeout, transient overload) before
  /// giving up - a single bad request shouldn't misread a title with tons of
  /// real availability as "no stream found". This retry is deliberately
  /// SKIPPED for [strict] (background availability probing, [AvailabilityProber]):
  /// that path already fails open on any error without needing a retry (it
  /// just skips recording that probe's result), and probing runs dozens of
  /// titles concurrently across every catalog row - retrying every failure
  /// there would multiply the request volume hitting AIOStreams and, under
  /// load, make the underlying slowness worse instead of better. [strict]:
  /// true also means a lookup failure (IMDb-id resolution or the AIOStreams
  /// request itself) is rethrown instead of being read as "no streams", so
  /// [AvailabilityProber] can fail open on it.
  Future<(List<StreamSource>, bool)> _fetchSources(PlaybackRequest request, {bool strict = false}) async {
    ...
    final meta = await Future.wait([
      strict ? _imdbId(request) : _imdbId(request, retry: true).catchError((_) => null),
    ...
    if (_aioUrl.isNotEmpty && imdb != null && imdb.isNotEmpty) {
      // Retry only on the player path (strict: false) - see the class-level
      // doc above for why background probing must NOT retry.
      final maxAttempts = strict ? 1 : 3;
  ...
  /// as "no stream found" - identical to a genuinely dead title - so on the
  /// player path a transient TMDB blip here is just as costly as one on the
  /// AIOStreams request itself, and [retry] (only ever passed true from
  /// there) absorbs it. Never enabled for background probing: that path
  /// already fails open on any error without needing a retry, and probing
  /// runs dozens of titles concurrently - retrying every failure there would
  /// multiply the request volume for no correctness benefit.
  Future<String?> _imdbId(PlaybackRequest request, {bool retry = false}) async {
    final type = request.isTvEpisode ? 'tv' : 'movie';
